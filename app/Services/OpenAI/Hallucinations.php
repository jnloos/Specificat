...
final class Hallucinations
{
    public static function filter(array $segments): array
    {
        return self::filterForLocale($segments);
    }

    public static function filterForLocale(array $segments, ?string $locale = null): array
    {
        $lines = self::commonForLocale($locale);

        return array_values(array_filter($segments, static function (string $segment) use ($lines): bool {
            ...
            foreach ($lines as $line) {
                if ($normalized === self::normalize($line)) {
                    return false;
                }
            }
    ...
    private static function commonHallucinations(): array
    {
        return [
            'en' => [
                'www.mooji.org',
     ...
     */
    private static function commonForLocale(?string $locale): array
    {
        $locale = $locale ? strtolower($locale) : 'en';
        $lines = self::commonHallucinations()[$locale] ?? [];
        $fallback = $locale === 'en' ? [] : (self::commonHallucinations()['en'] ?? []);

        return array_merge($lines, $fallback);
    }
